...
class CreateChat extends BaseApi
{
    ...
    public function msgs()
    {
        ...
        $msgList = Db::table('kt_gptcms_create_msg')->field('id,message,response')->where($where)->order('id asc')->select();
        $msgs = [];
    ...
    public function send()
    {
        ...
        $user = $this->user;
        if($user['status'] != 1){
            $this->outError('账号因异常行为进入风控，请联系客服解除风控！error:004');
        }
        ...
        $getMessages = $this->getMessages($message,$wid,$user,$model_id,$lang,$cmodel);
        $messages = $getMessages['messages'];
        $make_message = $getMessages['make_message'];
        //是否开启了key池
        ...
        if($chatmodel == "wxyy"){
            $callback = $this->getWxqfCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else if($chatmodel == "xfxh"){
            $callback = $this->getXfxhCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else if($chatmodel == "fastgpt"){
            $callback = $this->getFastCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else if($chatmodel == "chatglm"){
            $callback = $this->getChatglmCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else if($chatmodel == "minimax"){ 
            $callback = $this->getMiniMaxCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else if($chatmodel == "tyqw"){
            $callback = $this->getTyqwCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend);
        }else{
            $callback = $this->getCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend,$keysSwitch,$apiKey,$chatmodel);
        }
        ...
        $config = Db::table('kt_gptcms_gpt_config')->json(['openai','api2d','wxyy','tyqw','kltg','chatglm','linkerai','gpt4','api2d4','xfxh','fastgpt',"azure","minimax","txhy"])->where('wid',$wid)->find();
        if($config){
            switch ($chatmodel) {
                case 'gpt35':
    ...
    private function getMessages($message,$wid,$user,$model_id,$lang,$cmodel)
    {
        // 连续对话需要带着上一个问题请求接口
    ...
    private function getXfxhCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend){
        //返回的文字
        $response = ''; 
        $un_response = '';
        $callback = function($data) use ($message,$wid,$user,$vip,$cmodel,$make_message,$expend) {
            global $response;
            global $un_response;
    ...
    private function getWxqfCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
    ...
    private function getChatglmCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
    ...
    private function getMiniMaxCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
    ...
    private function getTyqwCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
    ...
    private function getFastCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
    ...
    private function getCallback($message,$wid,$user,$vip,$cmodel,$make_message,$expend,$keysSwitch,$apiKey,$chatmodel)
    {
        //返回的文字
        $response = ''; 
        $un_response = '';
        //不完整的数据
        $imperfect = '';
        $callback = function($ch, $data) use ($message,$wid,$user,$vip,$cmodel,$make_message,$expend,$keysSwitch,$apiKey,$chatmodel) {
            global $response;
            global $un_response;
            global $imperfect;
            $dataLength = strlen($data);
