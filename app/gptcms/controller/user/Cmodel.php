...
class Cmodel extends BaseUser
{
	public function list()
    {
        ...
        $res = Db::table("kt_gptcms_cmodel")->where('wid',$wid)->field('id,title,tp_url,desc,bz,xh,vip_status,status,c_time,classify_id,content,hint_content,defalut_question,defalut_reply');
    ...
    public function delete()
    {
    	...
    	$res = Db::table("kt_gptcms_cmodel")->where('wid',$wid)->where('id',$id)->delete();
    	if(!$res) return error('删除错误');
    	return success('删除成功');
    }
    public function importData(){
        $wid = Session::get('wid');
        $models = Db::table("kt_gptcms_cmodel")->where('wid',$wid)->select();
        if(!$models->isEmpty()) return error('已有模型,不可导入默认模型');
        $classify_id =  Db::table("kt_gptcms_cmodel_classify")->where('wid',$wid)->where('title','默认')->value('id');
        if(!$classify_id)  {
            $classify_id = Db::table("kt_gptcms_cmodel_classify")->insertGetId([
                "wid" => $wid,
                "title" => "默认",
        ...
        $res = Db::table("kt_gptcms_cmodel")->insertAll($cz);
    ...
    public function save()
    {
    	...
    	$res = Db::table("kt_gptcms_cmodel")->save($data);
    	return success('保存成功');
    }
    ...
    {   
        ...
        Db::table('kt_gptcms_cmodel')->where('id',$id)->where('wid',$wid)->update([
            'status' => $status,
            'u_time' => date("Y-m-d H:i:s"),
    ...
    public function classifyDelete()
    {
    	...
    	$res = Db::table("kt_gptcms_cmodel_classify")->where('wid',$wid)->where('id',$id)->delete();
    	if(!$res) return error('删除错误');
    	return success('删除成功');
    }
    public function classifySave()
    {
    	...
    	$data['u_time'] = date("Y-m-d H:i:s");
    	$res = Db::table("kt_gptcms_cmodel_classify")->save($data);
    	return success('保存成功');
    }
