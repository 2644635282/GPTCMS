...
//let host = 'https://chat1.xiaobolongtech.cn'
let host = location.origin	

const search = new URLSearchParams(location.search)
let wid = search.get('wid') || '1'
	...
	Vue.prototype.$u.http.interceptor.response = (res, config) => {
		if (res.statusCode == 401 || res.data.code == 401) {
			...
			if (res.status == 'success') {
				...
				if (res.status == 'error' && (res.msg == '无效的token' || res.msg == '用户不存在')) {
					uni.setStorageSync('clitoken', '')
					if (config.url.indexOf("api/users/getuserinfo") < 0) {
