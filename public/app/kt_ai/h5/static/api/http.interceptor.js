...
const install = (Vue, vm) => {
				...
				if(window.location.href.indexOf('localhost:')!=-1||window.location.href.indexOf('192.168.0.')!=-1) {
					...
					uni.setStorageSync('host','https://'+config.host)
				}
				...
				console.log(ext,'sssss')
				let wid2 = ext.wid || config.wid;
				let domain = ext.domain || config.host;
		
				uni.setStorageSync('host','https://'+domain)
				uni.setStorageSync('wid',wid2)
		
				// #endif


let host1 = uni.getStorageSync('host')
let wid1 = uni.getStorageSync('wid')



	console.log(uni.getStorageSync('host'),'ssssss')
	...
	Vue.prototype.$u.http.interceptor.request = (setConfig) => {
		setConfig.header['token']=uni.getStorageSync('h5Token')||''
		setConfig.header['wid'] = wid1;
	...
	Vue.prototype.$u.http.interceptor.response = (res) => {
		if (res.statusCode == 401 || res.data.code == 401) {
			...
			if (res.status == 'success') {
				...
				if(res.msg!='支付中...'&&res.msg!='暂未查询到用户'&&res.msg!='缺少参数wid'){
					vm.$u.toast(res.msg);
				}

				if(res.status == 'error'&&res.msg=='无效的token'){

					uni.setStorageSync('h5Token','')
						  ...
						  uni.login({
							...
							success: res => {
							  ...
							  ;(async () => {
								...
								wx.request({
									url:uni.getStorageSync('host')+'/gptcms/api/users/xcxlogin' , //仅为示例，并非真实的接口地址
									...
									success:(res)=>{
										...
										if(res.data.status=='success'){
											uni.setStorageSync('h5Token',res.data.data.token)
											uni.showToast({
												title: '重新登录成功',
												icon: 'success'
											});
											uni.$emit('updateDataMy')
										}
